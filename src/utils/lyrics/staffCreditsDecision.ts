...
export const resolveStaffDecision = (
    ...
    const blockLineCount = staffLines.length;
    ...
    const base = {
        blockLineCount,
        requiredSeconds: blockLineCount * minDwell + TAIL_MARGIN_SECONDS,
    ...
    if (windowSeconds >= base.requiredSeconds) {
        return {
            ...base,
            verdict: needsRetime(staffLines, firstLyric.startTime - TAIL_MARGIN_SECONDS, minDwell) ? 'retime' : 'keep',
            windowSeconds,
        };
    }
